function [layers,Xoutt,Youtt,Zoutt,Xt,Yt,Zt,layer_boundaries] = VPF_create_hippocampus_layers(sampled_img,subject_id,surf_path,T1_path,N_layers,rule)
Xoutt = {};
Youtt = {};
Zoutt = {};
Xt = {};
...
%TODO:
% *Clean up the ugly final loop.
% *Make the distinction between DG and rest of hippocampus for the case of merged
%  subfields. Maybe get rid of merging subfields and just average them after sampling.
%
...
for volume = 1:N_volumes
    ...
    for jj = 1:2 %hemispheres
        ...
        if contains(rule,'&') %either entire hippocampus or individual subfields merged together
            ...
            if ndims(input_image) > 3
                [layers{jj}(:,volume),Xoutt{jj}(:,volume),Youtt{jj}(:,volume),Zoutt{jj}(:,volume),Xt{jj}(:,volume),Yt{jj}(:,volume),Zt{jj}(:,volume)] = VPF_sample_layers (input_image(:,:,:,volume),layer_boundaries_sampled{1},N_layers);
            else
                 [layers{jj}(:,volume),Xoutt{jj}(:,volume),Youtt{jj}(:,volume),Zoutt{jj}(:,volume),Xt{jj}(:,volume),Yt{jj}(:,volume),Zt{jj}(:,volume)] = VPF_sample_layers(input_image,layer_boundaries_sampled{1},N_layers);
            end
            ...
            for subfield = 1:numel(mask)
                ...
                if strcmp(single_rule,'6')
                    ...
                    if ndims(input_image) > 3
                        [layers{jj,kk}(:,volume),Xoutt{jj,kk}(:,volume),Youtt{jj,kk}(:,volume),Zoutt{jj,kk}(:,volume),Xt{jj,kk}(:,volume),Yt{jj,kk}(:,volume),Zt{jj,kk}(:,volume)] = VPF_sample_layers(input_image(:,:,:,volume),layer_boundaries_sampled,1,ISDG);
                    else
                        [layers{jj,kk}(:,volume),Xoutt{jj,kk}(:,volume),Youtt{jj,kk}(:,volume),Zoutt{jj,kk}(:,volume),Xt{jj,kk}(:,volume),Yt{jj,kk}(:,volume),Zt{jj,kk}(:,volume)] = VPF_sample_layers(input_image,layer_boundaries_sampled,1,ISDG);
                    end
                else
                    ...
                    if ndims(input_image) > 3
                        [layers{jj,kk}(:,volume,subfield),Xoutt{jj,kk}(:,volume,subfield),Youtt{jj,kk}(:,volume,subfield),Zoutt{jj,kk}(:,volume,subfield),Xt{jj,kk}(:,volume,subfield),Yt{jj,kk}(:,volume,subfield),Zt{jj,kk}(:,volume,subfield)] = VPF_sample_layers(input_image(:,:,:,volume),layer_boundaries_sampled,N_layers,ISDG);
                    else
                        [layers{jj,kk}(:,volume,subfield),Xoutt{jj,kk}(:,volume,subfield),Youtt{jj,kk}(:,volume,subfield),Zoutt{jj,kk}(:,volume,subfield),Xt{jj,kk}(:,volume,subfield),Yt{jj,kk}(:,volume,subfield),Zt{jj,kk}(:,volume,subfield)] = VPF_sample_layers(input_image,layer_boundaries_sampled,N_layers,ISDG);
                    end
                end
            end
        end
...
function [layers,Xoutt,Youtt,Zoutt,Xt,Yt,Zt] = VPF_sample_layers(sampled_img,layer_boundaries,N_layers,ISDG)
Xoutt = [];%{};
Youtt = [];%{};
Zoutt = [];%{};
Xt = [];%{};
